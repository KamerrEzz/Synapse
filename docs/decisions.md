...
| D8 | Hybrid search = RRF (coseno + FTS español). FTS busca `título + contenido` con **OR** de lexemas (`to_tsquery`), no AND de `plainto_tsquery`. `p_embedding` como literal `[…]`. `score::float8`. Semántica solo si `<=>` < 0.42 | FTS + semántica; el AND de “de qué trata el archivo de X” no aparece en el PDF; el umbral evita que k-NN rellene basura | `0003`, `0006`, `0008`, `0009`, `0015`, `src/lib/ai/search.ts` |
...
| D15 | BYOK: cada usuario guarda proveedor + URL + modelos + API key. AES-256-GCM en Next (`SYNAPSE_APP_SECRET`). Tabla `user_openai_keys` sin SELECT para `authenticated`. UI solo ve last4. Sin fallback a env. El workspace elige `ai_key_mode`: `personal` (cada miembro) o `shared` (clave del owner en el servidor; el ciphertext no sale al cliente) | El usuario paga su uso; no hay clave del producto. Un owner puede pagar el del equipo sin regalar la key | `0005`/`0006`/`0014`, `src/lib/crypto/secret.ts`, `/api/openai-key`, `/api/workspace-ai-mode` |
...
| D21 | MCP Streamable HTTP en `/api/mcp`. Auth = token personal `syn_mcp_…` (hash SHA-256). Tools en inglés (lectura + CRUD wiki + estadísticas). Escritura de wiki: `plain_text` + `yjs_state` hex (el editor no lee el texto plano). `mcp_hybrid_search(p_user_id)` solo `service_role`. Sin OAuth | Los agentes no tienen cookie de sesión; `hybrid_search` exige `auth.uid()`; persistir solo `plain_text` lo pisa el editor | `src/mcp/*`, `0010_mcp_tokens.sql`. Detalle: [`mcp.md`](./mcp.md) |
| D22 | Estadísticas del workspace: agregados reales (índice, uso IA, costes USD estimados, CSV). `usage_events.metadata` guarda modelo/origen. Recharts en cliente. MCP: `get_workspace_stats` / `export_workspace_stats_csv` | BYOK: Synapse no factura; el coste se estima con lista pública OpenAI y usage de la API cuando existe. MCP usa `service_role` + `p_user_id` tras comprobar membresía | `src/app/.../stats`, `src/lib/stats/*`, `src/mcp/stats.ts`, `0011`–`0013` |
